...
echo "DESTINATION_SEC:     "$DESTINATION_SEC
...
cmsRun $PYCONFIG inputFiles=$INFILE 

ls

SRM_PREFIX="/eos/cms/" ; SRM_PATH=${DESTINATION#${SRM_PREFIX}} ; SRM_PATH_SECONDARY=${DESTINATION_SECONDARY#${SRM_PREFIX}} ;
if [[ $(wc -c reco_inMINIAOD.root | awk '{print $1}') -gt 700 ]]; then
    xrdcp reco_inMINIAOD.root root://eoscms.cern.ch//${SRM_PATH}/$OUTFILE
fi
...
[[ $(wc -c reco.root | awk '{print $1}') -gt 700 ]] && {
    xrdcp reco.root  root://eoscms.cern.ch//${SRM_PATH_SECONDARY}/$OUTFILE
}
